...
describe("DZapAdapter", function () {
  ...
  async function deployFixture() {
    ...
    const DZapAdapter = await ethers.getContractFactory("DZapAdapter");
    const adapter = await DZapAdapter.deploy(await router.getAddress(), ethers.ZeroAddress);
  ...
  it("pulls input from the caller and sends DZap output to the recipient", async function () {
  ...
  it("forwards leftover DZap output sitting on the adapter to the recipient", async function () {
    const { user, recipient, tokenIn, tokenOut, router, adapter } =
      await deployFixture();
  ...
  it("rejects a route target that is not the DZap router or Permit2", async function () {
    ...
    ).to.be.revertedWith("Invalid DZap target");
  });
});
