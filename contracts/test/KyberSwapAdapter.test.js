...
describe("KyberSwapAdapter", function () {
  ...
  async function deployFixture() {
    ...
    const KyberSwapAdapter = await ethers.getContractFactory("KyberSwapAdapter");
  ...
  it("pulls input from the caller and sends Kyber output to the recipient", async function () {
  ...
  it("rejects a route target that is not the Kyber router, executor proxy, or Permit2", async function () {
    ...
    ).to.be.revertedWith("Invalid Kyber target");
  });
});
