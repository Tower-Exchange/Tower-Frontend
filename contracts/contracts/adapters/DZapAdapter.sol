...
/**
 * @title DZapAdapter
 * @dev Executor-friendly adapter for DZap diamond swaps.
 *
 * TowerSwapExecutor approves this adapter, and the adapter then:
 * - pulls the ERC20 input from the executor,
 * - approves the DZap diamond and Permit2,
 * - forwards DZap's opaque swap calldata,
 ...
 */
contract DZapAdapter is Ownable, ReentrancyGuard {
    ...
    address public immutable dzapRouter;
    address public immutable permit2;

    constructor(address _dzapRouter, address _permit2) {
        require(_dzapRouter != address(0), "Invalid DZap router");
        require(_dzapRouter.code.length > 0, "DZap router is not a contract");
        if (_permit2 != address(0)) {
            require(_permit2.code.length > 0, "Permit2 is not a contract");
        }

        dzapRouter = _dzapRouter;
        permit2 = _permit2;
    }
    ...
    function swapExactInput(
        ...
        require(_isAllowedRouteTarget(routeTarget), "Invalid DZap target");
    ...
    function _isAllowedRouteTarget(address routeTarget) private view returns (bool) {
        return routeTarget == dzapRouter || (permit2 != address(0) && routeTarget == permit2);
    }

    function _approveSpenders(address token, uint256 amount) private {
        IERC20(token).safeApprove(dzapRouter, 0);
        IERC20(token).safeApprove(dzapRouter, amount);
    ...
    function _clearSpenders(address token) private {
        ...
        IERC20(token).safeApprove(permit2, 0);
        IPermit2AllowanceTransfer(permit2).approve(token, dzapRouter, 0, 0);
    }

    function _getRevertMsg(bytes memory returnData) private pure returns (string memory) {
        if (returnData.length < 4) {
            return "DZap route execution failed";
        }
        ...
        if (selector == 0x4e487b71) {
            return "DZap route execution panicked";
        }
