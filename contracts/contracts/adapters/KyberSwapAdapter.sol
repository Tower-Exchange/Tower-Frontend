 ...
 * @dev Executor-friendly adapter for KyberSwap Aggregator swaps on Arc.
 *
 * TowerSwapExecutor should allowlist this adapter as both a route target and
 * an approval spender. Do not allowlist Kyber's MetaAggregationRouterV2
 * directly on the executor. The adapter then:
 * - pulls the ERC20 input from the executor,
 * - approves Kyber's router (and Permit2 when present),
 * - forwards Kyber's opaque swap calldata,
 * - routes output back to the executor so it can pay the user and collect fees.
 */
contract KyberSwapAdapter is Ownable, ReentrancyGuard {
    ...
    constructor(address _kyberRouter, address _aggregationExecutor, address _permit2) {
        require(_kyberRouter != address(0), "Invalid Kyber router");
        require(_kyberRouter.code.length > 0, "Kyber router is not a contract");
        if (_aggregationExecutor != address(0)) {
            require(_aggregationExecutor.code.length > 0, "Kyber executor is not a contract");
        }
    ...
    function swapExactInput(
        ...
        require(_isAllowedRouteTarget(routeTarget), "Invalid Kyber target");
    ...
    function _isAllowedRouteTarget(address routeTarget) private view returns (bool) {
        return
            routeTarget == kyberRouter ||
            (aggregationExecutor != address(0) && routeTarget == aggregationExecutor) ||
            (permit2 != address(0) && routeTarget == permit2);
    }

    function _approveSpenders(address token, uint256 amount) private {
        IERC20(token).safeApprove(kyberRouter, 0);
        IERC20(token).safeApprove(kyberRouter, amount);
    ...
    function _clearSpenders(address token) private {
        ...
        IERC20(token).safeApprove(permit2, 0);
        IPermit2AllowanceTransfer(permit2).approve(token, kyberRouter, 0, 0);
    }

    function _getRevertMsg(bytes memory returnData) private pure returns (string memory) {
        if (returnData.length < 4) {
            return "KyberSwap route execution failed";
        }
        ...
        if (selector == 0x4e487b71) {
            return "KyberSwap route execution panicked";
        }
